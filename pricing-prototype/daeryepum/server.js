...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    try {
      ...
      } else if (pathname === '/api/admin/sync-order-raw' && (req.method === 'POST' || req.method === 'GET')) {
        // 어제 결제완료 + 어제 취소발생 답례품 주문 → Google Sheets 주문RAW 시트 append.
        //   URL: POST /api/admin/sync-order-raw (cron 호출)
        //         GET  /api/admin/sync-order-raw?start=YYYY-MM-DD&end=YYYY-MM-DD&dry_run=1 (수동 호출)
        //   환경변수:
        //     GOOGLE_SHEETS_CREDENTIALS_JSON: Service Account JSON 전체 (문자열)
        //     GOOGLE_SHEETS_SPREADSHEET_ID:   시트 ID
        //     GOOGLE_SHEETS_SHEET_NAME:       시트 이름 (예: '주문RAW')
        //   안정키 (H열) 기반 중복 방지 — 이미 시트에 있는 row 는 skip.
        if (!isSuperAdmin(session) && !(await hasRole(session, ['admin', 'operator']))) {
          return denyForbidden(res, 'admin/operator 필요');
        }
        try {
          const required = ['GOOGLE_SHEETS_CREDENTIALS_JSON', 'GOOGLE_SHEETS_SPREADSHEET_ID', 'GOOGLE_SHEETS_SHEET_NAME'];
          const missing = required.filter(k => !process.env[k]);
          if (missing.length > 0) {
            return json(res, { error: `환경변수 미설정: ${missing.join(', ')} — Docker Manager 등록 필요` }, 503);
          }
          const dryRun = parsed.query.dry_run === '1' || parsed.query.dry_run === 'true';
          // 기간 — 기본: 어제 0시 ~ 오늘 0시 (KST)
          let startStr = (parsed.query.start || '').trim();
          let endStr = (parsed.query.end || '').trim();
          if (!startStr || !endStr) {
            const todayKst = today();
            endStr = fmtDate(todayKst);
            startStr = fmtDate(addDays(todayKst, -1));
          }
          ...
          const pp = await getPool();
          // SQL: A주문일시 B상품명 C상품코드 D상품수량 E결제액 F주문사이트 G주문번호 H안정키 I변경구분
          //   (1) 어제 결제완료 (settle_date)
          //   (2) 어제 취소발생 (mod_date) — ⚠️ 운영자가 컬럼명 다르면 수정 필요
          //   CARD = custom_order, ETC = CUSTOM_ETC_ORDER
          //   채널 정규화: 바른손카드/바른손몰/쿠팡/네이버/기타 (더기프트 매핑 없음)
          //   취소 row 결제액 = 음수, 안정키 끝에 '|CANCEL'
          const result = await pp.request()
            .input('s', sql.VarChar, startStr)
            .input('e', sql.VarChar, endStr)
            .query(`
              SELECT
                CONVERT(varchar(19), 주문일시, 120) AS A_주문일시,
                상품명                              AS B_상품명,
                상품코드                            AS C_상품코드,
                상품수량                            AS D_상품수량,
                ROUND(결제액, 0)                    AS E_결제액,
                주문사이트                          AS F_주문사이트,
                주문번호                            AS G_주문번호,
                안정키                              AS H_안정키,
                변경구분                            AS I_변경구분
              FROM (
                -- (1) CARD 결제완료
                SELECT co.order_date AS 주문일시, c.Card_Name AS 상품명, c.Card_Code AS 상품코드,
                  coi.item_count AS 상품수량,
                  CAST(coi.item_sale_price AS float) * coi.item_count / ISNULL(NULLIF(c.Unit_Value, 0), 1) AS 결제액,
                  CASE WHEN si.SiteName IN ('바른손카드','바른손몰','쿠팡','네이버') THEN si.SiteName ELSE '기타' END AS 주문사이트,
                  CONCAT('CARD-', co.order_seq) AS 주문번호,
                  CONCAT('CARD-', co.order_seq, '|', c.Card_Code) AS 안정키,
                  '신규결제' AS 변경구분
                FROM custom_order co WITH (NOLOCK)
                INNER JOIN custom_order_item coi WITH (NOLOCK) ON co.order_seq = coi.order_seq
                INNER JOIN S2_Card c WITH (NOLOCK) ON coi.card_seq = c.Card_Seq
                ...
                WHERE (c.Card_Div = 'D01' OR c.Card_Code LIKE 'COM[_]%')
                  ...
                  AND co.settle_date >= @s AND co.settle_date < @e
                UNION ALL
                -- (2) CARD 취소발생 (mod_date 기준 — ⚠️ 컬럼명 확인 필요)
                SELECT co.order_date, c.Card_Name, c.Card_Code, coi.item_count,
                  -1 * CAST(coi.item_sale_price AS float) * coi.item_count / ISNULL(NULLIF(c.Unit_Value, 0), 1),
                  CASE WHEN si.SiteName IN ('바른손카드','바른손몰','쿠팡','네이버') THEN si.SiteName ELSE '기타' END,
                  CONCAT('CARD-', co.order_seq),
                  CONCAT('CARD-', co.order_seq, '|', c.Card_Code, '|CANCEL'),
                  '취소'
                FROM custom_order co WITH (NOLOCK)
                INNER JOIN custom_order_item coi WITH (NOLOCK) ON co.order_seq = coi.order_seq
                INNER JOIN S2_Card c WITH (NOLOCK) ON coi.card_seq = c.Card_Seq
                ...
                LEFT JOIN SiteInfo si ON comp.SALES_GUBUN = si.SiteCode
                WHERE (c.Card_Div = 'D01' OR c.Card_Code LIKE 'COM[_]%')
                  AND co.status_seq IN (3, 5)
                  AND co.mod_date >= @s AND co.mod_date < @e
                UNION ALL
                -- (3) ETC 결제완료
                SELECT o.order_date, c.Card_Name, c.Card_Code, oi.order_count,
                  CASE WHEN si.SiteName IS NULL
                  ...
                  END,
                  CASE WHEN si.SiteName IN ('바른손카드','바른손몰','쿠팡','네이버') THEN si.SiteName ELSE '기타' END,
                  CONCAT('ETC-', o.order_seq),
                  CONCAT('ETC-', o.order_seq, '|', c.Card_Code),
                  '신규결제'
                FROM CUSTOM_ETC_ORDER o WITH (NOLOCK)
                INNER JOIN CUSTOM_ETC_ORDER_ITEM oi WITH (NOLOCK) ON o.order_seq = oi.order_seq
                INNER JOIN S2_Card c WITH (NOLOCK) ON oi.card_seq = c.Card_Seq
                ...
                -- (4) ETC 취소발생
                SELECT o.order_date, c.Card_Name, c.Card_Code, oi.order_count,
                  -1 * (CASE WHEN si.SiteName IS NULL
                             THEN CAST(oi.card_sale_price AS float) * oi.order_count / ISNULL(NULLIF(c.Unit_Value, 0), 1) - ISNULL(o.coupon_price, 0)
                             ELSE CAST(oi.card_sale_price AS float) - ISNULL(o.coupon_price, 0)
                       END),
                  CASE WHEN si.SiteName IN ('바른손카드','바른손몰','쿠팡','네이버') THEN si.SiteName ELSE '기타' END,
                  CONCAT('ETC-', o.order_seq),
                  CONCAT('ETC-', o.order_seq, '|', c.Card_Code, '|CANCEL'),
                  '취소'
                FROM CUSTOM_ETC_ORDER o WITH (NOLOCK)
                INNER JOIN CUSTOM_ETC_ORDER_ITEM oi WITH (NOLOCK) ON o.order_seq = oi.order_seq
                INNER JOIN S2_Card c WITH (NOLOCK) ON oi.card_seq = c.Card_Seq
                LEFT JOIN SiteInfo si WITH (NOLOCK) ON o.company_Seq = si.CompayCode
                WHERE (c.Card_Div = 'D01' OR c.Card_Code LIKE 'COM[_]%')
                  AND o.status_seq IN (3, 5, 15)
                  AND o.mod_date >= @s AND o.mod_date < @e
              ) t
              ORDER BY 주문일시
            `);
          const rows = result.recordset;

          // dry_run 또는 row 0건이면 sheets 호출 skip + 결과만 응답
          if (dryRun) {
            data = { ok: true, dry_run: true, period: `${startStr} ~ ${endStr}`, fetched: rows.length, sample: rows.slice(0, 10) };
          } else if (rows.length === 0) {
            data = { ok: true, period: `${startStr} ~ ${endStr}`, fetched: 0, appended: 0, message: '어제 신규/취소 row 없음' };
          } else {
            // Google Sheets API
            ...
            // 1) 기존 시트 H열 (안정키) 조회 → 중복 방지
            const existing = await sheets.spreadsheets.values.get({
              spreadsheetId,
              range: `${sheetName}!H:H`,
            });
            const existingKeys = new Set((existing.data.values || []).flat().filter(Boolean));

            // 2) 신규 row 만 필터
            const newRows = rows.filter(r => !existingKeys.has(r.H_안정키));

            if (newRows.length === 0) {
              data = { ok: true, period: `${startStr} ~ ${endStr}`, fetched: rows.length, appended: 0, skipped_dup: rows.length, message: '모두 중복' };
            } else {
              // 3) append
              const values = newRows.map(r => [
                r.A_주문일시, r.B_상품명, r.C_상품코드,
                r.D_상품수량, r.E_결제액, r.F_주문사이트,
                r.G_주문번호, r.H_안정키, r.I_변경구분,
              ]);
              ...
              data = {
                ...
                skipped_dup: rows.length - newRows.length,
              };
            }
          }
        } catch (err) {
          console.error('[sync-order-raw] error:', err.message);
          data = { error: err.message, hint: 'mod_date 컬럼명이 다르면 SQL 수정 필요. 환경변수 GOOGLE_SHEETS_* 확인.' };
        }
